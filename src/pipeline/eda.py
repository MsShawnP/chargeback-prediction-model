...
EXPECTED_ROW_RANGES = {
    "retailer_chargebacks": (300, 700),
    "retailer_deductions": (1000, 3000),
    "retailer_shipments": (5000, 10000),
    "product_master": (20, 50),
    "distribution_log": (10, 1000),
    "retailer_deduction_codes": (5, 100),
}
...
def profile_date_ranges() -> None:
    ...
    probes = [
        ("retailer_shipments", "ship_date"),
        ("retailer_chargebacks", "chargeback_date"),
    ]
...
def profile_product_master_nulls() -> pd.DataFrame:
    ...
    try:
        total = int(query_to_df("SELECT COUNT(*) AS n FROM product_master").iloc[0]["n"])
    except Exception as exc:
    ...
    for col in target_cols:
        try:
            n_null = int(
                query_to_df(
                    f"SELECT COUNT(*) AS n FROM product_master WHERE {col} IS NULL"
                ).iloc[0]["n"]
            )
...
def check_chargeback_shipment_join() -> dict:
    ...
    try:
        total = int(
            query_to_df("SELECT COUNT(*) AS n FROM retailer_chargebacks").iloc[0]["n"]
        )
    except Exception as exc:
    ...
    try:
        matched = int(query_to_df("""
            SELECT COUNT(*) AS n
            FROM retailer_chargebacks c
            JOIN retailer_shipments s ON c.order_id = s.order_id
        """).iloc[0]["n"])
        ...
        if rate >= 0.5:
            print("  ✓ Match rate ≥ 50% — U4 can use order_id join")
        else:
    ...
    try:
        ...
        if rate_w >= 0.5:
            print("  ✓ Window join ≥ 50% — viable fallback if order_id join is weak")
    except Exception as exc:
